...
<body>
  ...
  ${(() => {
    ...
    if (summaryData.ot_notes) {
      const match = summaryData.ot_notes.match(/HOSPITAL STAY NOTES:?\s*([\s\S]*?)(?=INVESTIGATION SUMMARY|MEDICATIONS ON DISCHARGE|MEDICATIONS:|ADVICE|$)/i);
  ...
  ${(() => {
    ...
    // AI-written narrative interpretation of the labs (conservative summaries include an
    // "INVESTIGATION SUMMARY" section in ot_notes). Shown above the raw values.
    let invNarrative = '';
    if (summaryData.ot_notes) {
      const ns = summaryData.ot_notes.match(/INVESTIGATION SUMMARY:?\s*([\s\S]*?)(?=\nADVICE\b|$)/i);
    ...
    let invHtml = '';
    if (hasRealLabResults || hasRealInvestigationsText) {
    ...
    <div class="section">
      ...
      ${invNarrative ? `<div class="section-content" style="margin-bottom: 8px;">${invNarrative.replace(/\n/g, '<br>')}</div>` : ''}
      ${invHtml ? `<div class="section-content">${invHtml}</div>` : ''}
    </div>`;
  })()}
                    ...
                    <Button
                      ...
                      onClick={async (e) => {
                        ...
                        try {
                          ...
                          // Give the AI the lab/investigation data so it can write an
                          // Investigation Summary narrative (conservative summaries only).
                          if (isConservativeCase && investigations.trim() && !contentToSend.includes('INVESTIGATIONS')) {
...
- Never invent or write the name of any doctor, surgeon, consultant or staff member. Include a clinician's name only if it is explicitly present in the input; otherwise omit it.`
                          ...
                          // Surgical/HIMS keeps the strict original rule (no behaviour change).
                          // Conservative gets a rule that still bans fabricated FACTS but lets the
                          // model develop the provided data into full, detailed paragraphs instead
                          // of 1-2 short lines.
                          const factRule = isConservativeCase
                            ? `- Base the summary on the clinical data provided and develop it FULLY into the detailed multi-sentence paragraphs the template asks for. You MAY elaborate the provided information into thorough professional medical prose and describe, in general professional terms, the standard conservative management and clinical reasoning for the stated diagnosis. Do NOT fabricate specific factual values that were not given — no invented vital-sign numbers, laboratory values, dates, medication names or named events; write around those in general terms. Never write "not recorded", "not documented" or "not provided".`
                          ...
                          const response = await fetch(geminiGenerateContentUrl(import.meta.env.VITE_GEMINI_API_KEY), {
                            ...
                            body: JSON.stringify({
                              ...
                              generationConfig: {
                                ...
                                temperature: isConservativeCase ? 0.35 : 0.7,
                                maxOutputTokens: isConservativeCase ? 3500 : 2000
                              }
                            })
                          });
