...
export const useAyushmanRMOs = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [editingRMO, setEditingRMO] = useState<AyushmanRMO | null>(null);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const { data: ayushmanRMOs = [], isLoading } = useQuery({
    queryKey: ['ayushman-rmos'],
    queryFn: async () => {
      ...
      return (data || []) as AyushmanRMO[];
    }
  });

  const addMutation = useMutation({
    mutationFn: async (newRMO: Omit<AyushmanRMO, 'id' | 'created_at' | 'updated_at'>) => {
      const { data, error } = await (supabase as any)
        .from('ayushman_rmos')
        .insert([newRMO])
        .select()
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ayushman-rmos'] });
      toast({ title: "Success", description: "Ayushman RMO added successfully" });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: "Failed to add Ayushman RMO", variant: "destructive" });
    }
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<AyushmanRMO> }) => {
      const { data, error } = await (supabase as any)
        .from('ayushman_rmos')
        .update(updates)
        .eq('id', id)
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ayushman-rmos'] });
      toast({ title: "Success", description: "Ayushman RMO updated successfully" });
      setIsEditDialogOpen(false);
      setEditingRMO(null);
    },
    onError: (error: any) => {
      toast({ title: "Error", description: "Failed to update Ayushman RMO", variant: "destructive" });
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ayushman-rmos'] });
      toast({ title: "Success", description: "Ayushman RMO deleted successfully" });
    },
    onError: (error: any) => {
      toast({ title: "Error", description: "Failed to delete Ayushman RMO", variant: "destructive" });
    }
  });

  const filteredRMOs = ayushmanRMOs.filter((rmo: AyushmanRMO) =>
  ...
  const handleEdit = (rmo: AyushmanRMO) => {
    setEditingRMO(rmo);
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this Ayushman RMO?')) {
  ...
  const handleExport = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Ayushman RMOs');
      XLSX.writeFile(wb, `ayushman_rmos_export_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const { error } = await (supabase as any).from('ayushman_rmos').insert(records);
        if (error) { sonnerToast.error('Failed to import: ' + error.message); }
        else { sonnerToast.success(`Imported ${records.length} records`); queryClient.invalidateQueries({ queryKey: ['ayushman-rmos'] }); }
  ...
  return {
    ...
    ayushmanRMOs, isLoading, filteredRMOs,
